...
class WorkspaceService:
    ...
    async def create_workspace(
        ...
        dataset_ids = set(request.dataset_ids)
        model_ids = set(request.model_ids)
        found_dataset_ids = (
            set((await db.execute(select(Dataset.id).where(Dataset.id.in_(dataset_ids)))).scalars().all())
            if dataset_ids
            else set()
        )
        found_model_ids = (
            set((await db.execute(select(ModelRegistry.id).where(ModelRegistry.id.in_(model_ids)))).scalars().all())
            if model_ids
            else set()
        )
