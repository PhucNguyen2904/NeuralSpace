...
export function VersionTimeline({ versions }: { versions: ModelVersion[] }) {
  return (
    <div className="relative pl-6">
      <div className="absolute left-2 top-1 bottom-1 w-px bg-violet-200" />
      <div className="space-y-4">
        {versions.map((v) => (
          <div key={v.id} className="relative">
            <span className={`absolute -left-[17px] top-1.5 h-3 w-3 rounded-full border-2 ${v.current ? "border-violet-500 bg-violet-500" : "border-violet-500 bg-white"}`} />
